...
```
	 The summary is organized in following chapters:
	      1. Chapter 1: Introduction of Project & SMART Questionis
	      2. Chapter 2: Previous Research
	      3. Chapter 3: Description of Dataset
	      4. Chapter 4: EDA of Observations of Wildfires
	      5. Chapter 5: EDA of Annually and Monthly Statistics & Hypothesis Testing
	      6. Chapter 6: Correlation and Linear Model
...
Did the annual number of wildfires in California increase with an increase in the temperature between 1992-2015?  <br />

Did higher precipitation levels result in a smaller or fewer number of wildfires in California between 1992-2015?  <br />

Did a lower fire suppression expenditure in California result in larger wildfires between 1992-2015?  <br />

Did lower levels of soil moisture in California result in larger or more frequent wildfires between 1992-2015?
...
# Chapter 4: EDA and hypothesis testing for Wildfire Observations
HISTOGRAMS
...
# Chapter 6: Correlation and Liner Regression model
...
```
    ...
    From the EDA, we try to evaluate the wildfires in two cases: people-caused and other reason(usually natrual reason). The result suppose that fires number for both type(man-caused and other) has strong correlation with temperature and moisture. This implies that we could make model to evaluate the influence of nature factors to answer SMART question.



## 6.2 Smart Question: Did the higher temperature increase the cases of wildfire in a month?
...
## 6.3 Smart Question: Did high precipitation decreases the cases of wildfire in a month?
```{r}
...
## 6.4 Smart Question: Did low soilmoisture level increases the cases of wildfire in a month?
```{r}
...
After exploring the data and conducting our own tests, we came to several conclusions. After looking into the effect of lower/higher air temperature and rainfall over the years, they didn't seem to have much effect on the surface, which is why we decided to split up the wildfires into groups based on class and cause. When it comes to Class A (smaller) versus Class G (larger) fires, Class A are much more frequent, air temperature and rainfall are usually lower and soil moisture is usually higher. This conclusion, however, seems fairly elementary so we looked into wildfires by causes to investigate if conditions varied between them. During lightning-caused fires, air temperature and rainfall was higher and soil moisture was lower than during fires caused by equipment use and arson. During arson-caused fires, air temperature and rainfall were lower and soil moisture was higher than equipment use-caused fires. While arson is largely unpredictable and difficult to analyze, necessary precautions could be taken when it is raining to lower the chances that lightning causes fires. For example, removing dry leaves and foliage would be a good start. <br />

In addition, after looking into the budget spending by California, it seems that it has been rising considerably over the last two decades, but the frequency and intensity of fires over the years hasn't really changed. This begs the question- what exactly is the budget going towards, and where can some of it go instead? Had we been able to see how the budget was partitioned, we could have made recommendations based on our findings as to where that funding should really go. For now, we recommend the state of California to evaluate carefully how that money is being spent, find ways to keep the soil moist, and remove dry foliage from the ground.
