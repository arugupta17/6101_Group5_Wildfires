...
library(corrplot)
corresult=cor(day_summary[c(3,4,7,8,10,11)],method='spearman')
day_summary$latgroup=as.factor(day_summary$latgroup)
day_summary$longgroup=as.factor(day_summary$longgroup)
corrplot(corresult,method='number',type = 'lower', diag = TRUE)
model1=lm(n~tair_day_livneh_vic+latgroup+longgroup,data=day_summary)
summary(model1)
model2=lm(log(n)~tair_day_livneh_vic+soilmoist1_day_livneh_vic+latgroup+longgroup+tair_day_livneh_vic*latgroup+tair_day_livneh_vic*longgroup,data=day_summary)
summary(model2)
```