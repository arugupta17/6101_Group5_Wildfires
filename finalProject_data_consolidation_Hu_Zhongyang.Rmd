...
temp3=final_wildfire %>% count(FIRE_YEAR, DISCOVERY_DOY)
...
model1=lm(log(n)~tair_day_livneh_vic,data=day_summary)
summary(model1)

model3=lm(log(n)~tair_day_livneh_vic+soilmoist1_day_livneh_vic,data=day_summary)
summary(model3)
...
```{r}
temp6=aggregate(FIRE_SIZE~FIRE_YEAR+DISCOVERY_DOY,data=final_wildfire,sum)


joined3=merge(cleaned_airtemp,cleaned_rainfall,by.x=c('year','DOY'),by.y=c('year','DOY'))
joined3=merge(joined3,cleaned_soilmoisture,by.x=c('year','DOY'),by.y=c('year','DOY'))

joined3$DOY=as.numeric(joined3$DOY)

temp6=merge(temp6,joined3,by.x=c('FIRE_YEAR','DISCOVERY_DOY'),by.y=c('year','DOY'),all.x=T)
str(temp6)



result=cor(drop_na(temp6[c(3,4,6,7)]))
