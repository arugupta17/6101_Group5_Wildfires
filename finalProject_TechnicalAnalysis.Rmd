...
```
```{r results='markup'}
xkabledply(logitFireSizeN, title = paste("Logistic Regression :", format(formula(logitFireSizeN)) ))
```
```{r logitmodel}

## FIRE_SIZE ~ nature variables       - PEOPLE CAUSED (P)
logitFireSizeP <- glm(FIRE_SIZE ~ Avg_Temp + Avg_SoilMoisture + Avg_Rainfall, data = summary_peoplecaused)
summary(logitFireSizeP)
```
```{r results='markup'}
xkabledply(logitFireSizeP, title = paste("Logistic Regression :", format(formula(logitFireSizeP)) ))
```{r}
...
## LogTotal/n ~ nature variables

```{r logitmodel}
## LogTotal ~ nature variables      - NATURE CAUSED (N)
logitTotalN <- glm(n~ Avg_Temp + Avg_SoilMoisture + Avg_Rainfall, data = summary_nature)
summary(logitTotalN)
plot(logitTotalN)
```
```{r results='markup'}
xkabledply(logitTotalN, title = paste("Logistic Regression :", format(formula(logitTotalN)) ))

#predict(logitTotal)
```
```{r logitmodel}
## Logn ~ nature variables        - PEOPLE CAUSED (P)
logitnP <- glm(log(n)~ Avg_Temp + Avg_SoilMoisture + Avg_Rainfall, data = summary_peoplecaused)
summary(logitnP)
plot(logitnP)
```
```{r results='markup'}
xkabledply(logitnP, title = paste("Logistic Regression :", format(formula(logitnP)) ))
...
## FIRE_SIZE ~ nature variables       - NATURE CAUSED (N)
logitFireSizeN <- glm(FIRE_SIZE ~ Avg_Temp + Avg_SoilMoisture + Avg_Rainfall, data = summary_nature)
summary(logitFireSizeN)
```
```{r results='markup'}
xkabledply(logitFireSizeN, title = paste("Logistic Regression :", format(formula(logitFireSizeN)) ))
```
...
## Month ~ FIRE_SIZE + nature variables
```{r logitmodel}
## Month ~ FIRE_SIZE + nature variables      - NATURE CAUSED (N)
logitMonthN <- glm(month ~ FIRE_SIZE + Avg_Temp + Avg_SoilMoisture + Avg_Rainfall, data = summary_nature)
summary(logitMonthN)
```
```{r results='markup'}
xkabledply(logitMonthN, title = paste("Logistic Regression :", format(formula(logitMonthN)) ))
```
```{r logitmodel}
## Month ~ FIRE_SIZE + nature variables      - PEOPLE CAUSED (P)
logitMonthP <- glm(month ~ FIRE_SIZE + Avg_Temp + Avg_SoilMoisture + Avg_Rainfall, data = summary_peoplecaused)
summary(logitMonthP)
```
```{r results='markup'}
xkabledply(logitMonthP, title = paste("Logistic Regression :", format(formula(logitMonthP)) ))
```
