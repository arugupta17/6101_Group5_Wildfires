...
#summary(model_large)
xkabledply(model_large, title = "Logit Regression :", format(formula(model_large)))
...
#summary(model_large)
xkabledply(model_large, title = "Predicting Probability :", format(formula(model_large)))
...
#summary(model_case)
xkabledply(model_case, title = "Linear Regression :", format(formula(model_case)))
plot(model_case)

#Average Fire Size
model_avgsize=lm(log(FIRE_SIZE)~tair_day_livneh_vic++soilmoist1_day_livneh_vic,data=joined4)
#summary(model_avgsize)
xkabledply(model_avgsize, title = "Linear Regression :", format(formula(model_avgsize)))
plot(model_avgsize)

#Total Fire Area
model_totalarea=lm(log(totalarea)~tair_day_livneh_vic++soilmoist1_day_livneh_vic,data=joined4)
#summary(model_totalarea)
xkabledply(model_totalarea, title = "Linear Regression :", format(formula(model_totalarea)))
...
## 4.1 Decision Tree model for MultiClass Classification
...
## 4.2 predicting fire classes A and B using Decision Tree.
...
## 4.3 Prediction after tuning model fit.
```{r pressure, echo=FALSE}
